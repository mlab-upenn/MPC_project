...
pointCircleInArray = computeRoadEdges(s_start, s_end, circleRadius, roadHalfWidth, signEdge=-1, disc = 0.1) # np.array(pointCircleIn)
plt.plot(pointCircleInArray[:,0], pointCircleInArray[:,1], '-', color='C3', label='XF')
pointCircleOutArray = computeRoadEdges(s_start, s_end, circleRadius, roadHalfWidth, signEdge=1, disc = 0.1) # np.array(pointCircleOut)
...
plt.ylabel('Steering', fontsize=20)
...
plt.xlabel('Iteration~j', fontsize=20)
plt.ylabel('$Mean~Computational~Time~[s]', fontsize=20)
plt.legend()
...
if input == 'y':
	...
	s_end = xFeasible[0,-1] + 0.5
	pointCircleInArray = computeRoadEdges(s_start, s_end, circleRadius, roadHalfWidth, signEdge=-1, disc=0.1) # np.array(pointCircleIn)
	plt.plot(pointCircleInArray[:,0], pointCircleInArray[:,1], '--', color="k", label='X_F')
	pointCircleOutArray = computeRoadEdges(s_start, s_end, circleRadius, roadHalfWidth, signEdge=1, disc=0.1) # np.array(pointCircleOut)
	...
	plt.xlabel('Iteration~j', fontsize=20)
	plt.ylabel('Time~Steps~T^j', fontsize=20)
	plt.legend()
